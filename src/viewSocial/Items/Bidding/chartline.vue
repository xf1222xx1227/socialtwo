...
export default {
  ...
  methods: {
    getMonthData() {
      ...
      this.str = "";
      this.attribute = "";
      this.attribute += ",substring(c.time_release,1,7) as 'date' ";
      this.groupby = "";
      this.groupby += `,substring(c.time_release,1,7)`;
      this.str +=
        ` and substring(c.time_release,6,2) between '` +
        begin +
        `' and '` +
        end +
        `' and substring(c.time_release,1,4)='` +
        year +
        `' `;
      this.getData();
    },
    getYearData() {
      ...
      this.str = "";
      this.attribute = "";
      this.attribute += " ,substring(c.time_release,1,4) as 'date' ";
      this.groupby = "";
      this.groupby += `,substring(c.time_release,1,4)`;
      this.str +=
        ` and substring(c.time_release,1,4) between '` +
        begin +
        `' and '` +
        end +
        `' `;
    ...
    getDayData() {
      ...
      this.str = "";
      this.attribute = "";
      this.attribute += ",substring(c.time_release,1,10) as 'date' ";
      this.groupby = "";
      this.groupby += `,substring(c.time_release,1,10)`;
      this.str +=
        ` and substring(c.time_release,9,2) between '` +
        begin +
        `' and '` +
        end +
        `' and substring(c.time_release,1,7)='` +
        year +
        `' `;
      this.getData();
    },
    ...
    getData() {
      this.dataChart = [];
      this.$api
        .getOneBiddingUsersBidCategoryItems({
    ...
    showChart() {
      //   let chartDom =
      //     this.datadate.type == "year"
      //       ? document.getElementById("chartLine")
      //       : this.datadate.type == "month"
