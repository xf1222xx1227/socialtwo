<template>
  <div id="total">
    <div class="mybutton">
      <el-button class="mybutton1" @click="changeType('all')">总览</el-button>
      <el-button @click="changeType('year')">按年统计</el-button>
      <el-button @click="changeType('month')">按月统计</el-button>
      <el-button @click="changeType('day')">按日统计</el-button>
    </div>
    <div class="myselect">
      <div class="selectyear selectsdate" v-show="returndata.type == 'year'">
        <el-select class="selectwidth" v-model="byear" placeholder="请选择">
          <el-option
            v-for="item in optionsbyear"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
        至
        <el-select class="selectwidth" v-model="fyear" placeholder="请选择">
          <el-option
            v-for="item in optionsfyear"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
      </div>
      <div class="selectmonth selectsdate" v-show="returndata.type == 'month'">
        <el-select class="selectwidth" v-model="bmonth" placeholder="请选择">
          <el-option
            v-for="item in optionsbmonth"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
        至
        <el-select class="selectwidth" v-model="fmonth" placeholder="请选择">
          <el-option
            v-for="item in optionsfmonth"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
      </div>
      <div class="selectday selectsdate" v-show="returndata.type == 'day'">
        <el-select class="selectwidth" v-model="bday" placeholder="请选择">
          <el-option
            v-for="item in optionsbday"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
        至
        <el-select class="selectwidth" v-model="fday" placeholder="请选择">
          <el-option
            v-for="item in optionsfday"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
          </el-option>
        </el-select>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  data() {
    return {
      beginyear: 2020,
      beginmonth: 1,
      beginday: 1,

      year: 2020,
      month: 1,
      day: 1,

      type: "all",
      byear: 0,
      fyear: 0,
      bmonth: 1,
      fmonth: 1,
      bday: 1,
      fday: 1,
      optionsbyear: [],
      optionsfyear: [],
      optionsbmonth: [],
      optionsfmonth: [],
      optionsbday: [],
      optionsfday: [],

      returndata: { type: "all" },
    };
  },
  created() {
    // this.$emit("getdate", this.returndata);

    let date = new Date();

    let year = date.getFullYear();
    this.year = year;
    this.setbyearoption();
    this.setfyearoption(1);
    this.fyear = year;

    let month = date.getMonth() + 1;
    this.month = month;
    this.setbmonthoption();
    this.setfmonthoption(1);
    this.fmonth = month;

    let day = date.getDate();
    this.day = day;
    this.setbdayoption();
    this.setfdayoption(1);
    this.fday = day;
  },
  methods: {
    setbyearoption() {
      let data = [];
      for (let i = this.year; i >= this.beginyear; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "年";
        data.push(datas);
      }
      this.optionsbyear = data;
      this.byear = data[data.length - 1].value;
    },
    setfyearoption(...val) {
      let data = [];
      for (let i = this.year; i >= this.byear; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "年";
        data.push(datas);
      }
      this.optionsfyear = data;
      if (this.byear > this.fyear) {
        this.fyear = data[0].value;
      } else {
        this.returndata.begin = this.byear;
        this.returndata.end = this.fyear;
        if (val.length == 0) {
          this.$emit("getdate", this.returndata);
        }
      }
    },
    setbmonthoption() {
      let data = [];
      for (let i = this.month; i >= this.beginmonth; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "月";
        data.push(datas);
      }
      this.optionsbmonth = data;
      this.bmonth = data[data.length - 1].value;
    },
    setfmonthoption(...val) {
      let data = [];
      for (let i = this.month; i >= this.bmonth; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "月";
        data.push(datas);
      }
      this.optionsfmonth = data;
      if (this.bmonth > this.fmonth) {
        this.fmonth = data[0].value;
      } else {
        this.returndata.begin = this.bmonth;
        this.returndata.end = this.fmonth;
        if (val.length == 0) {
          this.$emit("getdate", this.returndata);
        }
      }
    },
    setbdayoption() {
      let data = [];
      for (let i = this.day; i >= this.beginday; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "日";
        data.push(datas);
      }
      this.optionsbday = data;
      this.bday = data[data.length - 1].value;
    },
    setfdayoption(...val) {
      let data = [];
      for (let i = this.day; i >= this.bday; i--) {
        let datas = {};
        datas.value = i;
        datas.label = i + "日";
        data.push(datas);
      }
      this.optionsfday = data;
      if (this.bday > this.fday) {
        this.fday = data[0].value;
      } else {
        this.returndata.begin = this.bday;
        this.returndata.end = this.fday;
        if (val.length == 0) {
          this.$emit("getdate", this.returndata);
        }
      }
    },
    changeType(type) {
      this.returndata.type = type;
      if (type == "all") {
      } else if (type == "year") {
        this.returndata.begin = this.byear;
        this.returndata.end = this.fyear;
      } else if (type == "month") {
        this.returndata.begin = this.bmonth;
        this.returndata.end = this.fmonth;
      } else {
        this.returndata.begin = this.bday;
        this.returndata.end = this.fday;
      }
      //   this.returndata.end = "1";
      this.$emit("getdate", this.returndata);
    },
  },
  watch: {
    byear(newval, val) {
      this.setfyearoption();
    },
    bmonth(newval, val) {
      this.setfmonthoption();
    },
    bday(newval, val) {
      this.setfdayoption();
    },
    fyear(newval, val) {
      if (this.returndata.type == "year") {
        this.returndata.begin = this.byear;
        this.returndata.end = this.fyear;
        this.$emit("getdate", this.returndata);
      }
    },
    fmonth(newval, val) {
      if (this.returndata.type == "month") {
        this.returndata.begin = this.bmonth;
        this.returndata.end = this.fmonth;
        this.$emit("getdate", this.returndata);
      }
    },
    fday(newval, val) {
      if (this.returndata.type == "day") {
        this.returndata.begin = this.bday;
        this.returndata.end = this.fday;
        this.$emit("getdate", this.returndata);
      }
    },
  },
};
</script>

<style lang="less" scoped>
#total {
  height: 50px;
  display: flex;
  flex-flow: row;
  .mybutton {
    display: flex;
    flex-flow: row;
    align-items: center;
    .el-button {
      margin-left: -10px;
      border-left: 1px solid rgb(123, 226, 100);
      border-radius: 0px;
    }
    .mybutton1 {
      margin-left: 10px;
      border-left: none;
    }
  }
  .myselect {
    // flex: 1;
    display: flex;
    flex-flow: row;
    .selectsdate {
      margin-left: 20px;
      display: flex;
      align-items: center;
      .selectwidth {
        width: 150px;
      }
    }
  }
  //   .selectstype {
  //     align-items: center;
  //     border: 1px solid blue;
  //   }
}
</style>